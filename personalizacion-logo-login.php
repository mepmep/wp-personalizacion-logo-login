...
add_action( 'customize_register', function( $wp_customize ) {
    // Registrar sección con prioridad para asegurar visibilidad
    $wp_customize->add_section( 'seccion_login_personalizado', array(
        'title'       => 'Login Personalizado',
        'priority'    => 25,
        'description' => 'Personaliza el logo que aparece en la pantalla de acceso (wp-login.php).',
    ));

    // Registrar ajuste con sanitización adecuada para URLs de imágenes
    ...
    // Registrar control de selección de imagen
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'logo_login_personalizado', array(
        ...
        'settings' => 'logo_login_personalizado',
    )));
});

// 2. LÓGICA DE VISTA PREVIA (INYECTAR INTERFAZ DE LOGIN EN EL PERSONALIZADOR)
add_action( 'wp_head', function() {
    if ( is_customize_preview() ) {
        ...
        <style type="text/css">
            ...
            body.customize-support #page, body.customize-support #wrapper, body.customize-support #content { display: none !important; }
...
// 3. CARGA REAL EN LA PÁGINA DE ACCESO (Lo que ve el usuario final en wp-login.php)
